    ...
    const checkStatus = async (): Promise<ScaffoldItem> => {
      return new Promise(async (resolve, reject) => {
        ...
        if (statusResponse.data.status === 'Completed') {
          ...
          setTimeout(() => resolve(checkStatus()), 2000); // Retry after a second
        } else {
          reject(new Error(statusResponse.data.error || `Error in generating scaffold from Task id: ${taskId} status: ${statusResponse.data.status}`));
        }
      });
    };
